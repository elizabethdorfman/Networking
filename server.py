...
class Server:
		def __init__(self, addr, port, timeout):
			...
			self.server_on = True
			print(f"Server started on {self.addr}:{self.port} with a timeout of {self.timeout} seconds.")

		def start_server(self):
			#Run server indefinitely
			while self.server_on:
				try:
					connection, client_address = self.server_socket.accept() #Waits for a client to connect
					self.last_connection_time = time.time()  # Updates the last connection time
					print(f"Connection from {client_address} at {self.last_connection_time}.")
					thread = threading.Thread(target=self.handle_request, args=(connection,)) #Handles connection in new thread
					thread.start() #Executes thread
				except timeout: #Handles timeout exception
					print(f"Server timed out– {self.timeout} second timeout period has passed.")
					self.stop_server()
