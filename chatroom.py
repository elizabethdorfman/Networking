...
class ServerTCP:
	...
	def accept_client(self):
		try:
			...
			ready, _, _ = select.select([self.server_socket], [], [], 1)  # 1 second timeout
			if ready:
	...
	def run(self):
		print(f"Launching chatroom server on address: {self.address} and port number: {self.server_port}")
...
class ClientTCP:
	...
	def receive(self):
		while not self.exit_receive.is_set():
			try:
				ready, _, _ = select.select([self.client_socket], [], [], 1)  # 1 second timeout
				if ready:
...
class ServerUDP:
	...
	def run(self):
		print(f"Launching chatroom server on address: {self.address}:{self.server_port}")
		print("Press CTRL+C to shut down the server.")
		print("Waiting for users to join...")
		while not self.run_event.is_set():
			try:
...
class ClientUDP:
	...
	def receive(self):
		while not self.exit_receive.is_set():
			try:
				ready = select.select([self.client_socket], [], [], 1)  # 1-second timeout
				if ready[0]:
